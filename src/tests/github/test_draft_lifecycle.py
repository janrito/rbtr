...
from .conftest import mock_comment
...
def test_show_side_and_commit_tags(
    draft_engine: Engine, side: str, commit_id: str, expected_tag: str
) -> None:
    save_draft(
        42,
        ReviewDraft(
...
def test_sync_no_local_no_remote_is_noop(draft_engine: Engine, mock_pr: PullRequest) -> None:
    ...
    assert mock_pr.create_review.call_count == 0
...
def test_sync_pulls_and_pushes(
    draft_engine: Engine, mock_pr: PullRequest, pending_review: MagicMock
) -> None:
    ...
    mock_pr.create_review.return_value = MagicMock(id=201)

    review_comments: dict[int, list[MagicMock]] = {
        99: [mock_comment(comment_id=50, path="b.py", line=20, body="Remote finding.")],
        201: [
            mock_comment(comment_id=300, path="a.py", line=10, body="Local finding."),
            mock_comment(comment_id=301, path="b.py", line=20, body="Remote finding."),
        ],
    }
    mock_pr.get_single_review_comments.side_effect = lambda rid: review_comments.get(rid, [])
...
def test_sync_remote_edit_accepted(
    ...
    from rbtr.github.draft import stamp_synced
    ...
    mock_pr.create_review.return_value = MagicMock(id=201)
    review_comments: dict[int, list[MagicMock]] = {
        99: [mock_comment(comment_id=50, path="a.py", line=10, body="Edited remotely.")],
        201: [mock_comment(comment_id=300, path="a.py", line=10, body="Edited remotely.")],
    }
    mock_pr.get_single_review_comments.side_effect = lambda rid: review_comments.get(rid, [])
...
def test_sync_remote_summary_adopted_when_local_empty(
    ...
    pending_review.body = "Remote summary."
    save_draft(
    ...
    review_comments: dict[int, list[MagicMock]] = {
        99: [],
        201: [mock_comment(path="a.py", line=1, body="C.")],
    }
    mock_pr.get_single_review_comments.side_effect = lambda rid: review_comments.get(rid, [])
...
def test_sync_conflict_warns(
    ...
    mock_pr.create_review.return_value = MagicMock(id=201)
    review_comments: dict[int, list[MagicMock]] = {
        99: [mock_comment(comment_id=50, path="a.py", line=10, body="Remote edit.")],
        201: [mock_comment(path="a.py", line=10, body="Local edit.")],
    }
    mock_pr.get_single_review_comments.side_effect = lambda rid: review_comments.get(rid, [])
...
def test_post_no_draft(draft_engine: Engine) -> None:
    cmd_draft(draft_engine, "post")
    assert "No draft to post" in "\n".join(output_texts(drain(draft_engine.events)))


def test_post_empty_draft(draft_engine: Engine) -> None:
    save_draft(42, ReviewDraft(summary="", comments=[]))
    cmd_draft(draft_engine, "post")
...
def test_post_invalid_event(draft_engine: Engine, arg: str) -> None:
    save_draft(
        42,
        ReviewDraft(
            summary="Review.",
...
def test_post_event_types(
    draft_engine: Engine, mock_pr: PullRequest, arg: str, expected_event: str
) -> None:
    save_draft(
        42,
        ReviewDraft(
    ...
    call_kw = mock_pr.create_review.call_args.kwargs
...
def test_post_sends_suggestion_in_body(draft_engine: Engine, mock_pr: PullRequest) -> None:
    save_draft(
        42,
        ReviewDraft(
            summary="Fix.",
...
def test_post_draft_loaded_from_disk(draft_engine: Engine, mock_pr: PullRequest) -> None:
    ...
    assert mock_pr.create_review.call_args.kwargs["body"] == "Disk review."


# ── Clear (/draft clear) ────────────────────────────────────────────


def test_clear_deletes_draft(draft_engine: Engine) -> None:
    save_draft(
        42,
        ReviewDraft(
...
def test_sync_translates_stale_comment_line(
    repo_draft_engine: Engine, mock_pr: PullRequest, draft_pr_target: PRTarget
) -> None:
    """Comment at base line 2 gets translated to head line 3."""
    mock_pr.head.sha = draft_pr_target.head_sha
    mock_pr.base.sha = draft_pr_target.base_commit
    save_draft(
        42,
        ReviewDraft(
            summary="Review.",
    ...
    mock_pr.get_single_review_comments.side_effect = lambda rid: []

    cmd_draft(repo_draft_engine, "sync")

    pushed = mock_pr.create_review.call_args.kwargs["comments"]
...
def test_sync_sends_side_per_comment(
    ...
    review_comments: dict[int, list[MagicMock]] = {201: []}
    mock_pr.get_single_review_comments.side_effect = lambda rid: review_comments.get(rid, [])
...
def test_sync_file_level_comments_pass_validation(
    draft_engine: Engine, mock_pr: PullRequest
) -> None:
...
def test_post_sends_side_and_commit_id(draft_engine: Engine, mock_pr: PullRequest) -> None:
...
def test_post_file_level_comments_not_rejected(draft_engine: Engine, mock_pr: PullRequest) -> None:
    ...
    text = "\n".join(output_texts(drain(draft_engine.events)))
    assert "posted" in text.lower() or "review" in text.lower()
    assert len(mock_pr.create_review.call_args.kwargs["comments"]) == 1
