...
def test_save_persists_new_messages(engine: Engine) -> None:
    """After save_new_messages, messages appear in the engine._store."""
    engine.state.message_history = [_user("hello"), _assistant("hi")]

    save_new_messages(engine, run_cost=0.01)

    loaded = engine._store.load_messages(engine.state.session_id)
    assert len(loaded) == 2
...
def test_save_is_incremental(engine: Engine) -> None:
    """Second save only inserts new messages, not duplicates."""
    engine.state.message_history = [_user("q1"), _assistant("a1")]
    save_new_messages(engine)

    engine.state.message_history.extend([_user("q2"), _assistant("a2")])
    save_new_messages(engine)
...
def test_save_noop_when_nothing_new(engine: Engine) -> None:
    """Calling save when nothing is new doesn't error or duplicate."""
    engine.state.message_history = [_user("q1")]
    save_new_messages(engine)
...
def test_save_attributes_cost_to_last_message(engine: Engine) -> None:
    """run_cost is written to the store on the last row."""
    engine.state.message_history = [_user("q"), _assistant("a")]
...
def test_new_resets_session_id(engine: Engine) -> None:
    """After /new, a new session_id is assigned."""
    old_id = engine.state.session_id

    engine.state.message_history = [_user("q1"), _assistant("a1")]
    save_new_messages(engine)

    # Simulate /new
    engine.state.message_history.clear()
    engine.state.session_id = engine._store.new_id()
    engine.state.saved_count = 0

    engine.state.message_history = [_user("q2")]
    save_new_messages(engine)
    ...
    assert engine.state.session_id in ids


# ── compaction resets saved count ────────────────────────────────────


def test_compaction_reset_allows_resave(engine: Engine) -> None:
    """After compaction resets saved_count, full history is re-persisted."""
    engine.state.message_history = [_user("q1"), _assistant("a1")]
    save_new_messages(engine)

    # Simulate compaction: replace history, reset count.
    engine.state.message_history = [_user("[summary]"), _user("q2")]
    engine.state.saved_count = 0
    save_new_messages(engine)
...
def test_session_context_on_stored_rows(engine: Engine) -> None:
    """Repo and model context appear in session listings."""
    engine.state.session_label = "testowner/testrepo — main"
    engine.state.message_history = [_user("q")]
    save_new_messages(engine)

    sessions = engine._store.list_sessions(
        repo_owner=engine.state.owner, repo_name=engine.state.repo_name
    )
    assert len(sessions) == 1
    assert sessions[0].session_label == "testowner/testrepo — main"
