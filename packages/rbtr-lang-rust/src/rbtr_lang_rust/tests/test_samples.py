"""rust sample extraction: the `samples/rust/` project through the real pipeline."""
...
def project() -> list[tuple[str, str]]:
    root = Path(__file__).parent / "samples" / "rust"
    return [
...
def chunks(project: list[tuple[str, str]]) -> list[Chunk]:
    ...
    for path, text in project:
        lang = manager.detect_language(path) or "rust"
...
def test_parses_cleanly(project: list[tuple[str, str]]) -> None:
    manager = get_manager()
    for path, text in project:
        grammar = manager.load_grammar(manager.detect_language(path) or "rust")
