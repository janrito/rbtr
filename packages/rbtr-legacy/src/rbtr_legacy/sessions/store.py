...
from rbtr_legacy.sessions.incidents import Incident
from rbtr_legacy.sessions.kinds import (
    Fact,
    Fragment,
    FragmentKind,
    FragmentStatus,
...
from rbtr_legacy.sessions.overhead import Overhead
...
from rbtr_legacy.sessions.stats import (
    GlobalStats,
    IncidentStats,
    OverheadStats,
    TokenStats,
...
class ResponseWriter:
    ...
    Created by `begin_response`.  The engine
    iterates PydanticAI stream events and calls `add_part`
    / `finish_part`.  The response is invisible to
    `load_messages()` until `finish` (or context-manager
    ...
    def add_part(self, index: int, part: ModelResponsePart) -> None:
        ...
        `status = 'in_progress'` until `finish_part` is called.
        """
...
class SessionStore:
    ...
    def _migrate(self, from_version: int) -> None:
        ...
        """
        from rbtr_legacy.sessions.migrations import backup, migrate_2026030301, migrate_2026030801
    ...
    def begin_response(
        ...
        *,
        ...
        returns a `ResponseWriter` for adding parts
    ...
    def reset_latest_compaction(self, session_id: str) -> int:
        ...
        Raises `ValueError` if messages were added after the
    ...
    def load_messages_with_ids(self, session_id: str) -> list[tuple[str, ModelMessage]]:
        ...
        Same as `load_messages` but returns
    ...
    def fact_scopes(self) -> list[str]:
        """Return all distinct scopes that have any facts (including superseded)."""
        rows = self._con.execute(_FACT_SCOPES_SQL).fetchall()
