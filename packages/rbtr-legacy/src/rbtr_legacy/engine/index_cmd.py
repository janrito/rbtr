...
from rbtr_legacy.events import ColumnDef, IndexCleared, TableOutput
from rbtr_legacy.index.models import Chunk, ChunkKind, Edge, EdgeKind
from rbtr_legacy.models import SnapshotTarget
...
def _emit_commit_table(
    ...
    from rbtr_legacy.git.objects import commit_log_between, resolve_commit
...
def _model(engine: Engine, model_id: str) -> None:
    ...
    try:
        from rbtr_legacy.index.embeddings import reset_model  # deferred: heavy native lib
...
def _search_diag(engine: Engine, query: str) -> None:
    ...
    from rbtr_legacy.index.search import classify_query, weights_for_query
