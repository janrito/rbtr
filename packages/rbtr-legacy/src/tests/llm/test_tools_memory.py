...
@pytest.fixture
def ctx_no_repo(store: SessionStore) -> RunContext[AgentDeps]:
...
@pytest.mark.anyio
async def test_visible_when_enabled(ctx: RunContext[AgentDeps]) -> None:
...
def test_insert_global(ctx: RunContext[AgentDeps], store: SessionStore) -> None:
    """A global fact is inserted with scope `'global'`."""
    result = remember(ctx, "Always use type hints.")
...
def test_insert_global_explicit(ctx: RunContext[AgentDeps], store: SessionStore) -> None:
    """Passing scope='global' explicitly works."""
    result = remember(ctx, "Use ruff.", scope="global")
    assert "Saved" in result
...
def test_insert_repo(ctx: RunContext[AgentDeps], store: SessionStore) -> None:
    """A repo-scoped fact resolves to `owner/repo`."""
    result = remember(ctx, "Uses Django 5.1.", scope="repo")
    assert "Saved" in result
...
def test_repo_scope_without_repo(ctx_no_repo: RunContext[AgentDeps]) -> None:
    """Repo scope without a connected repo returns an error."""
    result = remember(ctx_no_repo, "Some fact.", scope="repo")
    assert "no repository" in result.lower()


def test_invalid_scope(ctx: RunContext[AgentDeps]) -> None:
    """An unrecognised scope returns an error."""
    result = remember(ctx, "Some fact.", scope="team")
...
def test_supersede_by_content(ctx: RunContext[AgentDeps], store: SessionStore) -> None:
    """Superseding by exact content marks old fact and inserts new."""
    store.insert_fact(GLOBAL_SCOPE, "Python 3.12+", SESSION)

    result = remember(ctx, "Python 3.13+", scope="global", supersedes="Python 3.12+")
...
def test_supersede_content_not_found(ctx: RunContext[AgentDeps]) -> None:
    """Superseding with non-matching content returns an error."""
    result = remember(ctx, "New fact.", scope="global", supersedes="This does not exist.")
    assert "No active fact" in result


def test_supersede_content_wrong_scope(ctx: RunContext[AgentDeps], store: SessionStore) -> None:
    """Superseding content from a different scope returns an error."""
    store.insert_fact("acme/widgets", "Repo-specific fact.", SESSION)

    result = remember(ctx, "New global fact.", scope="global", supersedes="Repo-specific fact.")
    assert "No active fact" in result
    assert "global" in result


def test_supersede_strips_whitespace(ctx: RunContext[AgentDeps], store: SessionStore) -> None:
    """Leading/trailing whitespace in supersedes is stripped."""
    store.insert_fact(GLOBAL_SCOPE, "Use ruff.", SESSION)

    result = remember(ctx, "Use ruff with --fix.", scope="global", supersedes="  Use ruff.  ")
    assert "Superseded" in result
