"""reStructuredText sample extraction: the `samples/rst/` project through the pipeline.

The project includes a helpers.py the docs cross-reference, so the snapshot captures
the doc sections, the python chunks, and the rst->python reference edges.
"""
...
def project() -> list[tuple[str, str]]:
    root = Path(__file__).parent / "samples" / "rst"
    return [
...
def chunks(project: list[tuple[str, str]]) -> list[Chunk]:
    ...
    for path, text in project:
        lang = manager.detect_language(path) or "rst"
...
def test_emits_expected_kinds(chunks: list[Chunk]) -> None:
    """The sample exercises RST's doc-section and import (reference) chunks."""
    kinds = {c.kind for c in chunks}
    assert {ChunkKind.DOC_SECTION, ChunkKind.IMPORT} <= kinds


def test_parses_cleanly(project: list[tuple[str, str]]) -> None:
    manager = get_manager()
    for path, text in project:
        grammar = manager.load_grammar(manager.detect_language(path) or "rst")
