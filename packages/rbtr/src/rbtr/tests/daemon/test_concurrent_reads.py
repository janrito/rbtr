...
@pytest.fixture
def daemon_store(tmp_path: Path) -> Generator[IndexStore]:
...
def _wait_for_build_start(client: DaemonClient, repo_path: Path, deadline_s: float) -> None:
    ...
    Returns when `active_build` is set *or* the index exists;
...
def test_search_returns_promptly_during_live_build(
    ...
    with DaemonClient(running_daemon.runtime_dir) as client:
        ...
        for _ in range(10):
            ...
            # The daemon should respond promptly with either
            # results or INDEX_IN_PROGRESS.
            assert isinstance(search_resp, (SearchResponse, ErrorResponse))
            if isinstance(search_resp, ErrorResponse):
                assert search_resp.code in ("index_in_progress", "index_not_built")
