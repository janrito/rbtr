...
def test_fts_finds_hit(fts_hit: SearchScenario, store: IndexStore, head_ref: SnapshotRef) -> None:
    matched = store.fulltext_matches(fts_hit.query, within=[head_ref])
...
def test_fts_returns_empty(
    ...
    assert len(store.fulltext_matches(fts_empty.query, within=[head_ref])) == 0
...
def test_fts_scoped_to_repo(
    ...
    assert store.fulltext_matches("alpha", within=[repo_one_ref])["name"].to_list() == [
        "alpha_func"
    ]
    assert len(store.fulltext_matches("alpha", within=[repo_two_ref])) == 0
...
def test_shared_chunk_found_via_fts_from_both_repos(shared_chunk_store: IndexStore) -> None:
    ...
    r1 = store.fulltext_matches("shared", within=[SnapshotRef(repo_id=1, snapshot_sha="head")])
    r2 = store.fulltext_matches("shared", within=[SnapshotRef(repo_id=2, snapshot_sha="head")])
...
def test_match_similar_single_vector(store: IndexStore, semantic_ref: SnapshotRef) -> None:
    ...
    query_vec = [1.0, 0.0, 0.0, 0.0]
    result = store.similar_matches([query_vec], within=[semantic_ref], top_k=2)
    assert len(result) >= 2
...
def test_match_similar_picks_best_score(store: IndexStore, semantic_ref: SnapshotRef) -> None:
    ...
    result = store.similar_matches([vec_a, vec_b], within=[semantic_ref], top_k=2)
...
def test_match_similar_empty(store: IndexStore, head_ref: SnapshotRef) -> None:
    ...
    vec = [1.0, 0.0, 0.0, 0.0]
    assert len(store.similar_matches([vec], within=[head_ref], top_k=5)) == 0
...
def test_idf_neutralised_common_term(store: IndexStore, idf_ref: SnapshotRef) -> None:
    ...
    assert len(store.fulltext_matches("config", within=[idf_ref])) > 0
